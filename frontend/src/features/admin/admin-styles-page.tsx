...
import { useCreateStyle, useUpdateStyle, useDeleteStyle } from "./use-style-mutations";
...
export default function AdminStylesPage() {
  ...
  const query = useAdminStyles(page, limit, search);
  ...
  // --- LOGIKA UTAMA: HANDLE SAVE (CREATE / UPDATE) ---
  ...
  // --- LOGIKA UTAMA: HANDLE DELETE (Berdiri Sendiri) ---
  async function handleDelete(id: string) {
    const confirmed = confirm("Delete this style?");
    if (!confirmed) return;
  ...
  // --- PENGONDISIAN TOMBOL BATAL SAAT EDIT ---
          ...
          <tbody>
            {styles.length > 0 ? (
              styles.map((style: any) => (
                <tr key={style.id} className="border-b hover:bg-zinc-50/50">
                  <td className="p-3 text-sm text-zinc-500">{style.id}</td>
                  <td className="p-3 font-medium">{style.name}</td>
                  <td className="p-3 text-zinc-600">{style.slug}</td>
